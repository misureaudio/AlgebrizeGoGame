...
This essay is a revised version of an earlier draft. The author gratefully acknowledges detailed reviews (Go_v0-review_1_Gem.md, Go_v1-review_2_Cla.md, Go_v1-review_3_Gem.md, Go_v1-review_4_Cla.md, Go_v2-review_5_gpt.md, Go_v3-review_6_gpt.md), whose suggestions have shaped the framework presented here: the state space is now modeled as a finite DAG under positional superko (with history made explicit), liberties are defined via the coboundary operator, life and death is formalized through the Knaster–Tarski fixed point of Benson's monotone operator, and endgames are analyzed through combinatorial game theory (with the independence hypothesis explicitly stated).
...
An **eye** is an empty intersection v ∈ V_∅ whose four orthogonal neighbors (when they exist) are all stones of the same color, forming a cycle in the induced subgraph on those neighbors. Equivalently, v is an eye of a stone group c when v ∈ V_∅ and ∂_1^* 1_{\{v\}} = 0 in C_1(G_c; Z_2), where G_c is the subgraph induced by c ∪ {v}. In the language of relative homology, nontrivial classes in H_1(G_c, c; Z_2) provide a topological certificate of such enclosure.
...
*Proof.* Let e_1, e_2 be the two disjoint candidate eyes of c. By definition of candidate eye, each e_i is orthogonally surrounded by stones of c, so any liberty of c incident to e_i lies in a region enclosed by c. Since the eyes are disjoint, any opponent move can fill at most one of them; the other remains an empty point orthogonal-neighbor to c. Consequently, after any opponent move, c still retains at least one liberty, so c is never captured. This means that for every X ∈ L with c ⊆ X, we have f(X) ⊇ c. In particular, f(c) ⊇ c, so c is a post-fixed point of f. By the Knaster–Tarski theorem, every post-fixed point is contained in the greatest fixed point, i.e., c ⊆ gfp(f). □

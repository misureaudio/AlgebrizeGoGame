# An Algebraic Framework for the Game of Go — v3

## 0. Acknowledgment

This essay is a revised version of an earlier draft. The author gratefully acknowledges detailed reviews (Go_v0-review_1_Gem.md, Go_v1-review_2_Cla.md, Go_v1-review_3_Gem.md, Go_v1-review_4_Cla.md, Go_v2-review_5_gpt.md), whose suggestions have shaped the framework presented here: the state space is now modeled as a finite DAG under positional superko (with history made explicit), liberties are defined via the coboundary operator, life and death is formalized through the Knaster–Tarski fixed point of Benson's monotone operator (with a strengthened Lemma 4.2), and endgames are analyzed through combinatorial game theory (with the independence assumption spelled out).
...
**Corollary 2.2.** *Every game of Go starting from a legal position p terminates after at most D(p,ε) moves, and in particular after at most 3^{n^2} − ℓ(p) moves, where ℓ(p) is the number of stones on the board at p.*
...
An **eye** is an empty intersection v ∈ V_∅ whose four orthogonal neighbors (when they exist) are all stones of the same color, forming a cycle in the induced subgraph on those neighbors. Equivalently, v is an eye of a stone group c when v ∈ V_∅ and ∂_1^* 1_{\{v\}} = 0 in C_1(G_c; Z_2), where G_c is the subgraph induced by c ∪ {v}. In the language of relative homology, a true eye corresponds to a nontrivial element of H_1(G_c, c; Z_2).
...
**Lemma 4.2 (bridge to topology, strengthened).** *If a region R of empty intersections contains two disjoint H_1-eyes of a Black stone group c, then c ⊆ gfp(f); in particular, c is unconditionally alive.*

*Proof.* Let e_1, e_2 ∈ R be the two disjoint eyes. By definition of H_1-eyes, each e_i is orthogonally surrounded by stones of c, and any liberty of c that lies in R must be incident to one of these eyes. Since the eyes are disjoint, any opponent move can fill at most one of them; the other remains an empty point orthogonal-neighbor to c. Consequently, after any opponent move, c still retains at least one liberty in R, so c is never captured. This means that for every X ∈ L with c ⊆ X, we have f(X) ⊇ c. In particular, f(c) ⊇ c, so c is a post-fixed point of f. By the Knaster–Tarski theorem, every post-fixed point is contained in the greatest fixed point, i.e., c ⊆ gfp(f). □
...
**Remark 4.3.** The converse of Lemma 4.2 fails: a group may be unconditionally alive without possessing two disjoint eyes (e.g., a "bent four" shape whose liberties are sealed by shared walls). The lattice formulation is strictly more general than the classical "two eyes" heuristic, which is why we treat the eyes as a sufficient but not necessary certificate of life.

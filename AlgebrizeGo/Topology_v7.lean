...
-- Note: The hypothesis `h_libs` used here is a formally convenient strengthening
-- of the paper's Lemma 4.2. Rather than proving graph reachability to establish that
-- liberties remain enclosed, we assert the stronger (but sufficient) condition that
-- all liberties of `c` are exactly the candidate eyes `e1` and `e2`.
 ...
 -- We split into two cases based on our hypothesis: l = e1 or l = e2
  ...
  · -- Case 2: The liberty is e2
    ...
    · -- Prove the boundary of {e2} is in c
      ...
      rw [Set.mem_singleton_iff] at hv; subst hv
      exact he2.2 u h_adj
-- 5. THE STATE SPACE AND SUPERKO DAG (Section 2)

-- We assume a base relation 'R' that represents a valid board-state transition
-- (e.g., placing a stone and removing captures). We abstract this to focus on Superko.
variable (R : BoardState n → BoardState n → Prop)

-- Step 1: Define the GameState with History
structure GameState (n : ℕ) where
  pos : BoardState n
  history : List (BoardState n)

-- Step 2: Define the Superko Transition Relation (T)
-- A transition from state 'a' to 'b' is legal if:
-- 1. It is a valid base move (R a.pos b.pos)
-- 2. The new position has NEVER occurred in the history (b.pos ∉ a.history)
-- 3. The new history is exactly the old position prepended to the old history.
def LegalTransition (a b : GameState n) : Prop :=
  R a.pos b.pos ∧
  b.pos ∉ a.history ∧
  b.history = a.pos :: a.history

-- Step 3: Prove that history length strictly increases under a Legal Transition
lemma history_length_increases (a b : GameState n) (h : LegalTransition R a b) :
    a.history.length < b.history.length := by
  -- Unpack the transition definition to get the history update rule
  rcases h with ⟨_, _, h_hist⟩
  -- Substitute b.history with (a.pos :: a.history)
  rw [h_hist]
  -- The length of (x :: xs) is always strictly greater than the length of (xs)
  exact Nat.lt_succ_self a.history.length

-- Step 4: Prove Acyclicity (No Cycles)
-- A graph is acyclic if there is no path from a state back to itself.
-- We prove this by showing that if a path exists, the history length must have increased,
-- meaning the start state and end state cannot be the same!

-- Define the transitive closure of our transition (i.e., a path of any length >= 1)
def ReachablePath (a b : GameState n) : Prop :=
  Relation.TransGen (LegalTransition R) a b

-- Prove that any path strictly increases history length
lemma path_increases_length (a b : GameState n) (path : ReachablePath R a b) :
    a.history.length < b.history.length := by
  -- We induce on the TransGen path structure
  induction path with
  | single h_trans =>
      -- Base case: a path of length 1 (a single transition)
      exact history_length_increases R _ _ h_trans
  | tail _ h_trans ih =>
      -- Inductive step: path to some state 'c', then one step to 'b'
      have h_step := history_length_increases R _ _ h_trans
      exact Nat.lt_trans ih h_step

-- THE GRAND THEOREM: The Superko Graph is a DAG (No Cycles!)
theorem superko_is_dag (a : GameState n) : ¬ ReachablePath R a a := by
  -- Assume for contradiction that there is a path from 'a' back to 'a'
  intro h_cycle
  -- This implies the history length of 'a' is strictly less than the history length of 'a'
  have h_absurd := path_increases_length R a a h_cycle
  -- This is a contradiction in the natural numbers! (a < a is false)
  exact Nat.lt_irrefl a.history.length h_absurd
